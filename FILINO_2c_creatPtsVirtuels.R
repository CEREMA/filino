...
if (Cas!=0)
{
  ...
  if (Cas==3 | Cas==4)
  {
    ...
    if (file.exists(nomtrhydro)==T)
    {
      ...
      # # On ne garde que les tronçons au niveau de la surface
      # trhydro=trhydroTout[grep(trhydroTout$position_par_rapport_au_sol,pattern="0"),]
      # # On ne garde que les tronçons larges ou les canaux
      # naretenir=which(trhydro$nature=="Canal"| 
      #                   trhydro$classe_de_largeur=="Entre 5 et 15 m"|
      #                   trhydro$classe_de_largeur=="Entre 15 et 50 m"|
      #                   trhydro$classe_de_largeur=="Plus de 50 m")
      # trhydro=rbind(trhydro[grep(trhydro$classe_de_largeur,pattern="Entre 15 et 50 m"),],
      #               trhydro[grep(trhydro$classe_de_largeur,pattern="Plus de 50 m"),])  
      
      # Il va falloir traiter les canaux, ne pas traiter les tronçons étroits et voir pour les canaux en hauteur...
      
      # trhydro=do.call(rbind,
      #                 lapply(sort(unique(trhydro$liens_vers_cours_d_eau )),
      #                        function(x) {st_sf(data.frame(liens_vers_cours_d_eau =x),geometry=st_union(trhydro[which(trhydro$liens_vers_cours_d_eau ==x),]))}))
      ...
      # TRES TRES LONG avec buffer
      # trhydro=st_intersection(trhydro, st_buffer(Masque2,50))
      # combine
      
      #20230719
      # if (is.null(dim(trhydro)[1])==FALSE)
      # {
      #   tmp_=st_buffer(st_intersection(trhydro, Masque2),50)[,"Aire"]
      #   st_geometry(tmp_)="geom"
      #   tmp__=st_union(rbind(tmp_[,"Aire"],
      #                        Masque2[,"Aire"]))# Aire indiqué que pour la fusion aucune utilité
      #   trhydro=st_cast(st_intersection(trhydro,tmp__),'MULTILINESTRING')
      # }else{
      #   Cas=0
      #   browser()
      # }
      ...
      if (is.null(dim(trhydro)[1])==FALSE)
      {
        if (dim(trhydro)[1]>1)
        {
          ...
          trhydro=trhydro[which(st_length(trhydro)==max(st_length(trhydro))),]
  ...
  if (dim(PtsCSV)[1]>0)
  {
    ...
    leve=c(1,9,2)[which(is.na(match(c(1,9,2),unique(PtsCSV$Classification)))==F)]
    ...
    if (Cas==1 | Cas==2| Cas==3)
    {
      ...
      coup=max(1,round(dim(PtsCSV)[1]/1000000))
      ...
      {
        Gp4=ggplot()+
          geom_point(data=PtsCSV_sf,aes(x=Distance2,y=Z,colour=Classification2),size=0.25)+
          scale_color_manual(values = legClassification_tmp[as.numeric(levels(PtsCSV_sf$Classification2))])+
          geom_hline(aes(yintercept=AltPtsVirtuels),color=CoulPtsVirtuels)+
          xlim(min(PtsCSV_sf$Distance2),max(PtsCSV_sf$Distance2))+  
          theme(legend.title=element_blank(),legend.position = "right")
      }
    ...
    if (Cas==4)
    {
      ...
      PtsCSV_sf$Classification2=factor(PtsCSV_sf$Classification,levels=c(1,9,2))
      
      # prinicpe majeur de balayage et d'application de boxplot
      balayage=seq(min(PtsCSV_sf$Distance2)-pasb[1],max(PtsCSV_sf$Distance2)+pasb[1],pasb[1])
      ...
      xlabt=cbind("Distance2 - Toutes les rives","Distance2 - Rive 1","Distance2 - Rive 2")
      ...
      for (igp in c(1,2,3))
      {
        ...
        Gp=ggplot()
        Gp=Gp+
          geom_point(data=PtsCSV_sf_tmp,aes(x=Distance2,y=Z,colour=Classification2),size=0.5)+
          scale_color_manual(values = legClassification_tmp[as.numeric(levels(PtsCSV_sf_tmp$Classification2))])+
          geom_line(data=NUNIEAU,aes(x=balayage,y=Pente1),color=colo[igp],linetype="solid",size=1)
        if (Cas==3)
        {
          Gp=Gp+ 
            geom_line(data=NUNIEAU,aes(x=balayage,y=Pente2),color=colo[igp],linetype="dotdash",size=1)
        }
        Gp=Gp+  
          geom_line(data=NUNIEAU,aes(x=balayage,y=stats1),color="black",linetype="solid",size=0.25)+
          geom_line(data=NUNIEAU,aes(x=balayage,y=stats2),color="black",linetype="solid",size=0.25)+
          geom_line(data=NUNIEAU,aes(x=balayage,y=stats3),color="black",linetype="solid",size=0.25)+
          geom_line(data=NUNIEAU,aes(x=balayage,y=stats4),color="black",linetype="solid",size=0.25)+
          geom_line(data=NUNIEAU,aes(x=balayage,y=stats5),color="black",linetype="solid",size=0.25)+
          xlim(min(NUNIEAU$balayage),max(NUNIEAU$balayage))+
      ...
      Gp4=ggplot()+
        geom_point(data=PtsCSV_sf,aes(x=Distance2,y=Z,colour=Classification2),size=0.25)+
        scale_color_manual(values = legClassification_tmp[as.numeric(levels(PtsCSV_sf_tmp$Classification2))])+
        geom_line(Rives,mapping=aes(x=balayage,y=Rives_1),color=colo[2],linetype="solid",size=0.75)+
        geom_line(Rives,mapping=aes(x=balayage,y=Rives_2),color=colo[3],linetype="solid",size=0.75)+  
        geom_line(Rives,mapping=aes(x=balayage,y=Rives_1_et_2),color=colo[1],linetype="solid",size=1.25)+
        xlim(min(NUNIEAU$balayage),max(NUNIEAU$balayage))+
        ylim(min(Rives$Rives_1_et_2, na.rm = TRUE)-0.25,max(Rives$Rives_1_et_2, na.rm = TRUE)+0.5)+
        theme(legend.title=element_blank(),legend.position = "right")+
        annotate("text",x = Rives$balayage[1], y = Rives$Rives_1_et_2[1], 
                 label = Rives$Rives_1_et_2[1], hjust=1,vjust=1,size=4,color="green")+
        annotate("text",x = Rives$balayage[dim(Rives)[1]], y = Rives$Rives_1_et_2[dim(Rives)[1]],
    ...
    if (Cas>0)
    {
      ...
      nomPtsVirtLaz=file.path(rep_COURSEAU,paste0(raciSurfEau,"_PtsVirt.laz"))
