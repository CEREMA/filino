...
FILINO peut aussi fonctionner sur d'autres types de LiDAR, en France ou à l'étranger.
...
- **Description** : Création d'une table d'assemblage pour les fichiers `copc.laz`.
- **Objectif** : Organiser et référencer les fichiers LiDAR pour faciliter leur traitement ultérieur.
- **Utilité** :
  - Permet de faire la table d'assemblage des données chargées précédemment.
  - Une option permet de calculer le nombre de points de chaque dalle. En fonction des dalles, un ou plusieurs vols ont pu avoir lieu, ce qui entraîne parfois un très grand nombre de points. Ce nombre de points est utile pour déterminer sur combien de processeurs lancer l'étape de création des MNT (étape **11_07**).
- **La table d'assemblage dispose d'un style défini avec des actions QGIS** :
  - En cliquant sur un polygone, le fichier `copc.laz` associé à ce polygone s'ouvre.
  
  - Ce fichier peut aider à limiter ou augmenter le nombre de processeurs de calcul utilisables en fonction de votre matériel informatique.
  - Il sert aussi à voir les dalles qui auraient été mal chargées. Dans ce cas, supprimez la ou les dalles manuellement et relancez le chargement.
...
- **Des fichiers BUG sont créés dans le répertoire de stockage de votre LiDAR**, indiquant un problème, venant soit d'une dalle mal chargée, soit d'un problème de RAM sur votre PC. Supprimez-les si vous souhaitez relancer sur ces dalles et rechargez les dalles si elles sont mal téléchargées.
...
- **C'est une étape complexe sujette à de possibles bugs**, merci de les remonter aux développeurs. Votre demande ne sera pas forcément examinée.
...
- **C'est une étape complexe sujette à de possibles bugs**. Le travail manuel doit être irréprochable. Lors des bugs, des fichiers SIG indiquant les problèmes potentiels sont souvent créés, indiquant des conflits entre appareillage automatique et travail manuel.
...
- **Description** : Extraction des points LiDAR dans les zones VIDE/EAU dans les masques de type 2 et calcul des points virtuels appliqué aux masques de type 1.
- **Objectif** : Créer des points virtuels pour combler les zones VIDE/EAU et améliorer la précision du MNT.
- **De nombreux dossiers et fichiers sont créés**. Des fichiers `copc.laz` sont créés avec des points LiDAR virtuels ayant des classifications dépendant de leur type.
...
- **Un fichier est créé, vous pouvez l'ouvrir dans QGIS**. Un style lui est associé avec des actions qui permettent d'ouvrir les fichiers `laz` ou le répertoire pour voir en détail les calculs effectués. Il est majeur de vérifier cette table avant de lancer des calculs ultérieurs de MNT.
