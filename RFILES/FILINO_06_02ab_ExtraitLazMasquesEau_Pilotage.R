...
if (length(n_int)>0)
{
  if (Supp_PtsVirt_copc_laz==1)
  {
    ...
    Sys.sleep(0.1);unlink(Liste_PtsVirt_copc_laz)
  }  
  ...
  if (Etap2[1]==1)
  {
    ...
    nb_proc=min(floor(0.6*detectCores()),nb_proc_Filino_[6],dim(TA)[1])
    if(nb_proc==0)
    {
...
if (Etap2[1]==1)
{
  cat("Etape 'a': Dans le dossier ",file.path(dsnlayer,NomDirSurfEAU),"\n")
  cat("Vérifier si des fichiers SurfEAU_xxxnomdalleLidarxxx.vide existent.\n")
  cat("Deux solutions.\n")
...
if (Etap2[3]==1)
{
  cat("Etape 'c': Dans le dossier ",file.path(dsnlayer,NomDirSurfEAU),"\n")
  cat(" Vérifier si des fichiers .jpg ne sont pas à des valeurs de poids 0 - sinon supprimer et relancer\n")
  cat("\n")
  ...
  cat("   - Si besoin, reprendre le travail manuel relancer à partir de l'étape 'FILINO_05_01c_MasqueEau.R'\n")
  cat("     sans rien supprimer, la gestion est normalement automatique\n")
  cat("\n")
