FILINO_00c_TA=function(chem_routine,dsnlayerTA,nomlayerTA,extension,paramXYTA,qgis_process,Doss_ExpRastCount,pdal_exe)
{
  cat("\014")
  ...
  if (length(listeLAZ)>0)
  {
    ...
    if (length(MutliTA)>0)
    {
      #---- Initialisation si la variable n'existe pas
      if (exists("CalcPixel")==F){CalcPixel=0}
      
      #---------------- CALCUL PIXEDL SI RASTER
      ...
      #---- Initialisation si la variable n'existe pas
      if (exists("CalcNptsLAZ")==F){CalcNptsLAZ=0}
      ...
      if (CalcNptsLAZ==1 & substr(Gagne_MultiTA$NOM[1],nchar(Gagne_MultiTA$NOM[1])-3,nchar(Gagne_MultiTA$NOM[1]))==".laz")
      {
        ...
        if (nb_proc==0)
        {
          ...
          for (ilaz in 1:nrow(Gagne_MultiTA))
          {
            ...
            FILINO_00c_TA_NbrePOINTS(chem_routine,ilaz,Gagne_MultiTA,Doss_ExpRastCount,NomTIF,pdal_exe)
          }
        }else{
          cat("------ ",nb_proc            ," CALCULS MODE PARALLELE -------------\n")
          require(foreach)
