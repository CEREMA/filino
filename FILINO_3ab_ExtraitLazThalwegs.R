...
for (iTA in 1:length(dsnTALidar))
{
  ...
  if (length(n_int)>0)
  {
    ...
    MasquesBuf=st_read(file.path(dsnlayer,NomDirMasque,paste0("Masques1_",racilayerTA,".gpkg")))
    ...
    if (Etap3[2]==1)
    {
      ...
      # for (i_CoursEau in c("COURDEAU0000002000804226"))
      {
        ...
        dir.create(file.path(dsnlayer,rep_COURSEAU))
        ...
        NomLaz_tmp=paste0(raciPCoursEau,"_",racilayerTA,".laz")
        cat("###############################################################\n")
        print(NomLaz_tmp)
